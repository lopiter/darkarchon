...
if ! [[ "$KIND" =~ ^(claude|agy)$ ]] && [ -n "$RESUME_ID" ]; then
    echo "NOTE: '$NAME' is a $KIND worker — only claude and agy can resume a conversation." >&2
    echo "      Respawning fresh." >&2
    RESUME_ID=""
fi
if [ "$FRESH" -eq 0 ] && [ -z "$RESUME_ID" ] && [[ "$KIND" =~ ^(claude|agy)$ ]]; then
...
if [ -n "$RESUME_ID" ]; then
    if [ "$KIND" = "agy" ]; then
        echo "Revived '$NAME' with its previous conversation (agy --conversation $RESUME_ID)."
    else
