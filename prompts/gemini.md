# Gemini Worker Overlay

You are a **Gemini CLI** worker. Everything in the team contract applies, with
one substitution: you have **no `mcp__darkarchon__*` tools**. Wherever the
contract names one, run the shell equivalent with your shell tool instead. Your
...
the contract describes. Use your `write_file` tool for the result file a
dispatch asks for — it must exist on disk before you print the DONE marker.
