...
notify() {
    ...
    # and interrupts the running turn (live-verified 1.0.5). A gemini pane
    # queues typed text into its composer, where it would ride along with the
    # next prompt instead of being read. Leave the message queued instead —
    # the worker's turn-end hook (lib/grok-state-hook.sh Stop /
    # lib/gemini-state-hook.sh AfterAgent) sees it outstanding when the turn
    # ends and keeps the model working until it drains. `outstanding` +
    # `renotify` remain the manual recovery path.
    case "$(worker_kind "$to")" in grok|gemini)
        local st
...
is_group_address() {
    case "$1" in
        @all|@idle|@claude|@codex|@grok|@gemini|@cwd:*) return 0 ;;
        *) return 1 ;;
    esac
}
...
case "$cmd" in
        ...
        case "$to" in
            @*)
                if ! is_group_address "$to"; then
                    ...
                    echo "  known: @all @idle @claude @codex @grok @gemini @cwd:<dir>" >&2
                    exit 1
                fi
    ...
    *)
        ...
        echo "  <to> may be a worker name or a group: @all @idle @claude @codex @grok @gemini @cwd:<dir>" >&2
        exit 1
        ;;
esac
