...
#   @agy           every agy (Antigravity CLI) worker
...
notify() {
    ...
    # and interrupts the running turn (live-verified 1.0.5). (agy is fine: it
    # queues a submitted line as the NEXT prompt, live-verified 1.2.2.) Leave
    # the message queued instead — the worker's Stop hook
    # (lib/grok-state-hook.sh) sees it outstanding when the turn ends and
    # keeps the model working until it drains. `outstanding` + `renotify`
    # remain the manual recovery path.
    case "$(worker_kind "$to")" in grok)
        local st
...
is_group_address() {
    case "$1" in
        @all|@idle|@claude|@codex|@grok|@agy|@cwd:*) return 0 ;;
        *) return 1 ;;
    esac
}
...
case "$cmd" in
        ...
        case "$to" in
            @*)
                if ! is_group_address "$to"; then
                    ...
                    echo "  known: @all @idle @claude @codex @grok @agy @cwd:<dir>" >&2
                    exit 1
                fi
    ...
    *)
        ...
        echo "  <to> may be a worker name or a group: @all @idle @claude @codex @grok @agy @cwd:<dir>" >&2
        exit 1
        ;;
esac
