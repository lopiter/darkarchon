...
```bash
$DARKARCHON_HOME/lib/spawn-worker.sh [--kind claude|codex|grok|gemini] [--env K=V] <name> <cwd> [<role>]
```
...
1. User said which ("as codex", "a claude worker", "grok으로", "gemini로") → pass `--kind`.
...
3. codex needs `codex login` (or `OPENAI_API_KEY`) first, or every turn 401s.
   grok needs a one-time interactive login (`~/.grok/auth.json`). gemini needs
   a one-time `gemini` sign-in (`~/.gemini/oauth_creds.json`) or
   `GEMINI_API_KEY`, and gemini-cli ≥ 0.59 (older builds lack hooks/--resume).
4. codex workers are task-executors only: no team contract, no darkarchon
   MCP tools (`ask`, `mailbox_send`). grok and gemini workers get the full
   contract (grok via `--rules`, gemini via a SessionStart hook) and use the
   shell equivalents (`lib/ask.sh`, `lib/mailbox.sh`); a message sent to a
   busy grok/gemini worker is held until its turn ends (a Stop / AfterAgent
   hook then makes it drain) — never typed mid-turn.

**Invite** — register a pane the user already has running:

```bash
$DARKARCHON_HOME/invite-worker.sh [--kind claude|codex|grok|gemini] <name> <session:window> [<role>]
```
...
- A revived worker is a new process, so charter, hooks, heartbeat and MCP tools
  are reattached. claude and gemini workers get their conversation back
  (`--resume`); codex and grok always restart fresh. A pane the user relaunched by hand has none of those, which is
...
`@grok`, `@gemini`, `@cwd:<dir>`. The sender is never included in its own group send. Group
